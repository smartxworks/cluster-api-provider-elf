...
func TestMachineCollection(t *testing.T) {
	t.Run("SortedByCreationTimestamp", func(t *testing.T) {
		t.Run("should return the same number of machines as are in the collection", func(t *testing.T) {
			...
			g.Expect(sortedMachines).To(gomega.HaveLen(len(collection)))
			g.Expect(sortedMachines[0].Name).To(gomega.Equal("machine-1"))
			g.Expect(sortedMachines[len(sortedMachines)-1].Name).To(gomega.Equal("machine-5"))
			g.Expect(collection.Oldest().Name).To(gomega.Equal("machine-1"))
		})
	})
	t.Run("SortedByDeletionTimestamp", func(t *testing.T) {
		t.Run("should return the same number of machines as are in the collection", func(t *testing.T) {
			...
			g.Expect(sortedMachines).To(gomega.HaveLen(len(collection)))
			g.Expect(sortedMachines[0].Name).To(gomega.Equal("machine-1"))
			g.Expect(sortedMachines[len(sortedMachines)-1].Name).To(gomega.Equal("machine-8"))
			g.Expect(collection.OldestDeletionTimestamp().Name).To(gomega.Equal("machine-1"))
		})
	})
	t.Run("Difference", func(t *testing.T) {
		t.Run("should return the collection with elements of the second collection removed", func(t *testing.T) {
			...
			// does not mutate
			g.Expect(collection.Names()).To(gomega.ContainElement("machine-1"))
			g.Expect(c3.Names()).To(gomega.ConsistOf("machine-1"))
		})
	})
	t.Run("Names", func(t *testing.T) {
		t.Run("should return a slice of names of each machine in the collection", func(t *testing.T) {
			g := gomega.NewWithT(t)
			g.Expect(New().Names()).To(gomega.BeEmpty())
			g.Expect(FromMachines(machine("1"), machine("2")).Names()).To(gomega.ConsistOf("1", "2"))
		})
	})
}
...
func TestMachinesLowestVersion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := gomega.NewWithT(t)
			g.Expect(tt.machines.LowestVersion()).To(gomega.Equal(tt.expected))
		})
	}
}
