	...
	Context("Reconcile VM status", func() {
		It("should shut down the VM when configurations was modified and VM is running", func() {
			...
			*vm.Vcpu += 1
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(logBuffer.String()).To(ContainSubstring("The VM configurations has been modified, shut down the VM first and then restore the VM configurations"))
		...
		It("should power off the VM when configurations was modified and shut down failed", func() {
			...
			*vm.CPU.Cores += 1
			...
			Expect(ok).To(BeFalse())
			Expect(err).NotTo(HaveOccurred())
			Expect(logBuffer.String()).To(ContainSubstring("The VM configurations has been modified, power off the VM first and then restore the VM configurations"))
			expectConditions(elfMachine, []conditionAssertion{{infrav1.VMProvisionedCondition, corev1.ConditionFalse, clusterv1.ConditionSeverityInfo, infrav1.PowerOffReason}})
		})
		...
		It("should restore the VM configurations when configurations was modified", func() {
			...
			*vm.CPU.Sockets += 1
			...
			Expect(ok).To(BeFalse())
			Expect(err).NotTo(HaveOccurred())
			Expect(logBuffer.String()).To(ContainSubstring("The VM configurations has been modified, and the VM is stopped, just restore the VM configurations to expected values"))
		...
		It("should power off the VM configurations when configurations and VM is suspended", func() {
			...
			*vm.Vcpu += 1
			...
			Expect(ok).To(BeFalse())
			Expect(err).NotTo(HaveOccurred())
			Expect(logBuffer.String()).To(ContainSubstring("The VM configurations has been modified"))
			expectConditions(elfMachine, []conditionAssertion{{infrav1.VMProvisionedCondition, corev1.ConditionFalse, clusterv1.ConditionSeverityInfo, infrav1.PowerOffReason}})
		})
	})
